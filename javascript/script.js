...
const API_KEY1 = [
  `09f34dfde082ce1b964aeba567e3ecaab58fff794ea4607015ef0709449211a1`,
];
...
tabs.forEach((mode) =>
  mode.addEventListener("click", (e) => popularLoanBooksFilter(e))
);
...
const popularLoanBooksRender = () => {
  const popularLoanBooksHTML = popularLoanBooksList
    ...
    .join("");
  document.getElementById("yb-popular-loan-books").innerHTML =
    popularLoanBooksHTML;
};
...
const paginationRender = () => {
  ...
  if (firstPage >= 6) {
    paginationHTML = `<a class="page-link" onclick = "moveToPage(1)" aria-label="First-Page">
      ...
      </a><a class="page-link" onclick = "moveToPage(${
        page - 1
      })" aria-label="Previous">
  ...
  if (lastPage < totalPages) {
    paginationHTML += `<a class="page-link" onclick = "moveToPage(${
      page + 1
    })" aria-label="Next">
...
const getBookhj = async (isbn13) => {
  const url2 = new URL(
    `http://data4library.kr/api/srchBooks?authKey=${API_KEY2}`
  );
